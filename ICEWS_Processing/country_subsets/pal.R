#### Create Palestinian Dissident Network using automated actors ####
library(dplyr)
library(tidyr)
library(stringr)
library(readr)
...
icews <- read_csv("~/Dissertation Data/networkcreation/ICEWS_Processing/icews_groups.csv")

#fix NAs - can't do this in the read command b/c they differ across columns
icews[,c(2:18,20:21)][icews[,c(2:18,20:21)]=="NA"] <- NA


#### Recode Individuals on Source Side ####
...
#remove individuals who aren't affiliated with a dissident group
#pal.diss <- subset(pal.diss, is.na(src.groups)==T | src.groups %in% groups$names)

pal.diss$alt.src <- ifelse(is.na(pal.diss$src.groups)==T, pal.diss$alt.src, pal.diss$src.groups)

#remove extraneous groups
pal.diss <- subset(pal.diss, alt.src!="Palestinian Territory, Occupied" & alt.src!="c(\"Govt\", \"Palestinian Territory, Occupied\")" & alt.src!="Riyad Mansour" & alt.src!="Muhammad Abu Khdeir")

#code a few that weren't in the dictionary
...
#### Create to dissident actor-months ####
...
frame <- group.months %>% 
  group_by(alt.src) %>% 
  summarize(start=min(ym),end=max(ym)) %>% 
  rowwise() %>% 
...
#### Recode Individuals on Target Side ####
...
#remove individuals who aren't affiliated with a dissident group
#pal.diss$conmo <- paste(pal.diss$tgt.groups, pal.diss$ym, sep="")

#pal.diss <- subset(pal.diss, is.na(tgt.groups)==T | tgt.groups %in% group.months$conmo)
...
#write
write_csv(pal.diss,"~/Dropbox/Prospectus/Diss Network Events/palestine.csv")
